...
| # | Sheet | Contents |
|---|---|---|
| 1 | **Summary** | **Page 1.** Four blocks in this order: KPI metric table · audit-trail measures · By category · By age bucket · By brand. Every cell is a formula over Episode Detail — nothing hardcoded. Layout detailed in §12.0. |
| 2 | **Filters & Definitions** | Resolved filter payload in plain English, provenance, and the §2 formula definitions verbatim. This is what stops the file being misquoted once it's circulating. |
| 3 | **Loss by Reason** | Family → code hierarchy, all 33 codes. Columns: Episodes, Peak Qty Short, Open Value, Delayed Revenue, Revenue-Days Delayed, Lost Revenue, Avg Days Delay. |
| 4 | **Episode Detail** | One row per episode — the workhorse sheet. Column list in §12.1. |
| 5 | **Event Log** | The append-only ledger for the filtered episodes, one row per event. The audit trail in flat form. |
| 6 | **By Item** | SKU ranking by Revenue-Days Delayed, with brand, peak shortfall and repeat-episode count. |
| 7 | **By Account** | Main account → branch rollup (via `AcumaticaCustomer.parent_acumatica_id`), matching the dashboard tree. |
| 8 | **Acumatica Bridge** | The §7.2 reconciliation, variance lines itemised, pass/fail gate. |
| 9 | **Lookups** | Brand prefix map and all thresholds (as-at date, promised lead time, write-off threshold, ESCALATE value floor, high-value floor). Every formula reads its parameters from here. |

### 12.0 Summary sheet layout — page 1

Mirrors the order-level summary the business already reads, re-cut to the episode grain. Five blocks:

**Block 1 — Metric / Value / Note**

| Metric | Definition |
|---|---|
| Open backorder episodes (order × item) | `COUNTIFS(State="Open", Excluded="No")` — the base grain |
| Open back orders (unique) | `SUM(Order 1st open row)` — DISTINCT order_nbr, the figure comparable to Acumatica's order count |
| Open items (unique SKUs) | `SUM(Item 1st open row)` |
| Total order value on back order (KES) | `SUMIFS(Line Order Value)` — **short lines only**, see the reading note below |
| Open / at-risk value (KES) | `SUMIFS(Revenue at Risk, State="Open")` |
| Value already fulfilled (KES) | `SUMIFS(Value Fulfilled)` |
| Overall value fill rate | Fulfilled ÷ total line value |
| Episodes with ZERO qty shipped | Hard stockouts — nothing has left the warehouse |
| Open value on those episodes (KES) | |
| Episodes older than 7 working days | Threshold read from Lookups, not hardcoded |
| Open value older than 7 working days (KES) | |
| Flagged ESCALATE | Aged **and** above the value floor |

**Block 2 — Audit-trail measures** (what the Acumatica screen cannot tell you): Resolved episodes in period · Avg time to resolve (numeric **and** as `5 Days Delay`) · Longest still open · Delayed revenue · Revenue-days delayed · Lost revenue · of which PRODUCTION family · Reason coverage % · Verified start dates %.

**Block 3 — By category** · Category | Episodes (all) | Open orders | Order value | Open value | Fill rate, plus Total.

**Block 4 — By age bucket** · Age bucket | Lower | Upper | Open episodes | Open orders | Open value. Lower/Upper are **editable blue parameter cells** so Commercial can re-band without a rebuild.

**Block 5 — By brand** · Brand | Episodes (all) | Open SKUs | Order value | Open value | Fill rate | Lost revenue | Revenue-days delayed | Avg days delay, plus an `(unmapped)` row that **must always read 0** — anything landing there means an Inventory ID prefix is missing from the brand map.

Blocks 3, 4 and 5 are three independent cuts of the same population, so **their totals must agree with each other and with Block 1**. That mutual agreement is the cheapest correctness check in the workbook — build it into the acceptance test.

**Column-header honesty matters here.** "Episodes (all)" spans open *and* resolved; "Open orders" and "Open SKUs" count open only. Labelling both simply "Orders" in the same table is how a reader ends up quoting 20 when the answer is 14.
...
- Columns for the join back to `AcumaticaSyncLog` (opened / resolved sync run id) stay in the export. They are how you debug a disputed row six weeks later.
- **Brand** sits immediately after Inventory ID, derived by `INDEX`/`MATCH` on the 3-character prefix against the Lookups brand map (`SIF`→Sifa, `COS`→Cosy, `FAY`→Fay, `TIS`→Tishu Poa, `COW`→Cow & Gate). In production, read brand from the item master instead and keep the prefix map only as a fallback — a code that doesn't parse must surface as `(unmapped)`, never be silently dropped.
- **Escalation Flag** is a formula, not a stored value: `ESCALATE` when working days > write-off threshold **and** at-risk > value floor; else `High value` above the high-value floor; else `Aged`; else `Monitor`. All three thresholds live on Lookups.
- Three value columns exist so the fill-rate maths is auditable rather than derived in a pivot: **Line Order Value** (`order_qty × price`), **Revenue at Risk** (`backorder_qty × price`), **Value Fulfilled** (`shipped_qty × price`).
...
- [ ] The Excel export's `SUM(Order 1st open row)` equals the API's `open_orders`, and `SUM(Item 1st open row)` equals `open_items`. No fractional results.
- [ ] On the Summary sheet, the Totals of **By category**, **By age bucket** and **By brand** each equal the Block 1 KPI figures for episodes, open orders and open value. Three independent cuts, one population.
- [ ] The `(unmapped)` brand row reads 0. A deliberately broken Inventory ID prefix makes it non-zero and fails the test.
- [ ] Every threshold used anywhere in the workbook resolves to a cell on Lookups. A grep for hardcoded `7`, `25000` or `100000` inside a formula fails the review.
