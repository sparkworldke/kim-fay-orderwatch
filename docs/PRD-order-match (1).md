...
> **Order type scope (v1.0):** Order Match only operates on **Sales Orders (prefix `SO`)**. Credit Notes (prefix `RC`) and Quotes (prefix `QO`) are explicitly out of scope and must never be pulled, surfaced, or matched against. See Guardrails §15 "Order Type Filtering" for enforcement rules.
...
| 6 | Future support for Credit Notes and Quotes is a separate phase (see §16 Open Questions) and requires its own design pass — do not silently extend matching to these types without a spec update. |
...
| 7 | Credit Note (RC) and Quote (QO) matching — separate phase scope, timeline, and design owner? | Product | TBD |
