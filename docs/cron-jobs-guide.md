...
|---------|---------|-----------------|-----------------|-------|
| `email-sync-3h` | `orderwatch:email-sync` | `0 */3 * * *` (every 3 hours) | Yes | Same-day Outlook sync only — resumes from last check watermark within today (not full history) |
...
| `sales-order-status-sync` | `orderwatch:sales-order-status-sync` | `12,42 * * * *` (every 30 min) | Yes | Status updates + delete local SOs missing from Acumatica (14-day lookback) |
| `sales-order-prune-missing` | `orderwatch:sales-order-prune-missing` | `30 1,7,13,19 * * *` (every 6h) | Yes | Wider prune of local SOs deleted/not found in Acumatica (60-day lookback) |
| `inventory-sync-{wh}` | `orderwatch:inventory-sync --warehouse=…` | Morning from **08:30**, midday from **12:00**, **+30 min per warehouse** | Yes | Per-warehouse stock: `DTC`, `FGS`, `FGS2`, `FGS2 RETURNS`, `MSA`, `EXPORT`, `PRMS`, `RMS1`, `TRMS` |
| `inventory-sync-5h` | `orderwatch:inventory-sync` | paused (manual) | No | Legacy all-warehouse full sync |
...
php artisan orderwatch:inventory-sync --source=manual --warehouse=FGS
php artisan orderwatch:inventory-sync --source=manual --job-key=inventory-sync-fgs --warehouse=FGS
php artisan orderwatch:inventory-sync --source=manual --full --warehouse=DTC
...
| `inventory-sync-{wh}` | 2 hours | Morning + midday stock waves (30 min stagger between warehouses) |
| `inventory-sync-5h` | — | Legacy all-warehouse job (paused) |
