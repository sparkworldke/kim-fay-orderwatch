...
## 13. Troubleshooting — KeyNotFoundException on $select / $expand

> **Error seen in logs:** `System.Collections.Generic.KeyNotFoundException` in `SelectPathSegmentTokenBinder.TryBindAsDeclaredProperty`

### Root Cause

This error means one or more field names in your `$select` or `$expand` query **do not exist in the custom endpoint schema** for `IpayV2/22.200.001`. Acumatica custom endpoints only expose the fields explicitly mapped when the endpoint was built in **Web Service Endpoints (SM207060)**. Any field name not in that map throws a `KeyNotFoundException` at the OData parser level — the entire request fails, not just the missing field.

This is **not** a permissions error and not a data error. The field name is simply not registered in the endpoint's entity definition.

---

### Affected Queries in Your Logs

| Query | Suspected missing fields |
|---|---|
| `SalesOrder` fill rate | `CurrencyID`, `Details/Description`, `Details/LineNbr` |
| `SalesOrder` backorders | `CustomerName`, `ScheduledShipmentDate`, `RequestedOn` |
| `StockItem` inventory | `SalesPrice`, `DefaultPrice`, `QtyOnHand`, `QtyAvailable`, `TotalQtyOnHand` |

---

### Step 1 — Discover exactly which fields are exposed

Hit the `$adHocSchema` endpoint for each entity. This returns the complete list of fields your endpoint version actually supports:
...
Compare the returned field list against every field in your `$select`. Any field in your query that is absent from the schema is the culprit. Do this **before** any other step — it tells you exactly what to fix.

---

### Step 2 — Binary search to isolate the bad field

If `$adHocSchema` is not available on your build, strip `$select` down to the minimum and add fields back one at a time until the request breaks:

```
# Baseline — does expand work at all?
GET /entity/IpayV2/22.200.001/SalesOrder?$top=5&$expand=Details

# Add header fields incrementally
?$select=OrderNbr
?$select=OrderNbr,Status
?$select=OrderNbr,Status,CustomerID
?$select=OrderNbr,Status,CustomerID,CurrencyID   ← breaks here? CurrencyID not mapped

# Add detail fields incrementally
?$select=OrderNbr&$expand=Details&$select=OrderNbr,Details/InventoryID
...continue until break
```

The first request that returns `KeyNotFoundException` identifies the field to remove or add to the endpoint.

---

### Step 3 — Fix: extend the IpayV2 endpoint in Acumatica

> Go to: **Main Menu → System → Integration → Web Service Endpoints (SM207060)**

1. Open endpoint `IpayV2`, version `22.200.001`
2. Click **Extend Endpoint** → assign a new version (e.g. `22.200.002`) to avoid breaking anything using the current version
3. Under **SalesOrder** entity, add missing header fields:
   - `CustomerName`
   - `CurrencyID`
   - `ScheduledShipmentDate`
   - `RequestedOn`
4. Open the **Details** child entity under SalesOrder, add:
   - `InventoryID`, `OrderQty`, `ShippedQty`, `OpenQty`, `UnitPrice`, `WarehouseID`, `UOM`, `LineNbr`, `Description`
5. Under **StockItem** entity, add:
   - `SalesPrice`, `DefaultPrice`, `QtyOnHand`, `QtyAvailable`, `TotalQtyOnHand`
6. Click **Validate** — fix any warnings before saving
7. Click **Publish**
8. Update your `BASE_URL` path from `22.200.001` → `22.200.002`

> **Why extend rather than edit in place?** Editing an existing endpoint version changes the contract for any other integration already using it. Extending creates a new version that only your updated code uses.

---

### Step 4 — Fallback: use the Default endpoint to unblock yourself now

While waiting for the endpoint fix to go through, switch temporarily to the standard Acumatica Default endpoint which maps all standard fields:

```
# Current (broken)
/entity/IpayV2/22.200.001/SalesOrder?...

# Temporary fallback
/entity/Default/23.200.001/SalesOrder?...
```

The Default endpoint uses `23.200.001` (not `22.200.001`) — confirm your instance's available Default version with:

```
GET /entity/
```

This returns a list of all available endpoint names and their versions.

---

### Safe Working Queries — Minimum Viable $select

Use these as your known-good baselines while you confirm which fields are mapped.

**SalesOrder — fill rate (conservative):**
```
GET /entity/IpayV2/22.200.001/SalesOrder
  ?$top=100
  &$skip=0
  &$filter=Date ge datetimeoffset'2026-06-01T00:00:00' and Date le datetimeoffset'2026-06-23T23:59:59' and Status ne 'Completed'
  &$expand=Details
  &$select=OrderNbr,Status,CustomerID,Details/InventoryID,Details/OrderQty,Details/ShippedQty,Details/OpenQty,Details/UnitPrice
```

Fields removed from your original query: `CurrencyID`, `CuryID`, `Details/WarehouseID`, `Details/UOM`, `Details/LineNbr`, `Details/Description` — add back one at a time to confirm which survive.

**SalesOrder — backorders (conservative):**
```
GET /entity/IpayV2/22.200.001/SalesOrder
  ?$filter=Status eq 'Backorder'
  &$expand=Details
...
Fields removed: `CustomerName`, `ScheduledShipmentDate`, `RequestedOn` — these are frequently not mapped in custom endpoints.

**StockItem — minimal safe:**
```
GET /entity/IpayV2/22.200.001/StockItem
  ?$top=100
  &$select=InventoryID,Description,ItemClass,DefaultUOM,BaseUOM,IsStockItem,DefaultWarehouseID
```

Fields removed: `SalesPrice`, `DefaultPrice`, `QtyOnHand`, `QtyAvailable`, `TotalQtyOnHand` — quantity fields on StockItem commonly require additional inventory views to be mapped in the endpoint.

> **Note on quantity fields:** `QtyOnHand` and `QtyAvailable` on StockItem are computed from inventory tables, not stored on the item record itself. Some Acumatica builds require these to be explicitly enabled in the endpoint via a separate **Inventory** sub-entity or a custom field mapping. If the endpoint extension above doesn't resolve them, check with your Acumatica admin whether the inventory summary view is enabled for the endpoint.

---

### Defensive Code Pattern

Never assume a field is available. Wrap `$select` construction to make it easy to swap fields in and out:
...
const SALES_ORDER_SELECT = [
  ...
  'Status',
  'CustomerID',
  // 'CurrencyID',           // ← comment out until confirmed mapped
  // 'CustomerName',         // ← comment out until confirmed mapped
].join(',');
...
const DETAILS_SELECT = [
  ...
  'Details/OpenQty',
  'Details/UnitPrice',
  // 'Details/WarehouseID',  // ← comment out until confirmed mapped
  // 'Details/LineNbr',      // ← comment out until confirmed mapped
].join(',');

const url = `${BASE_URL}/entity/IpayV2/22.200.001/SalesOrder` +
            `?$top=100&$skip=${skip}` +
            `&$filter=${filter}` +
            `&$expand=Details` +
            `&$select=${SALES_ORDER_SELECT},${DETAILS_SELECT}`;
```

This also means when you extend the endpoint and re-test, you can uncomment lines rather than hunt through concatenated query strings.

---

### Guardrail added to Section 12

> **API Guardrail #16 — Validate $select against endpoint schema before deploying.**
> Run `GET /entity/{endpoint}/{version}/{entity}/$adHocSchema` in any new environment before assuming your field list is valid. A field available in `Default` is not guaranteed to be mapped in a custom endpoint. Always binary-search failing `$select` queries rather than guessing — the error message names the parser class, not the offending field.

---

*Generated: June 2026 | Acumatica REST API v23.200.001*
*Sections: Authentication · Backorders · Sales Order Lines · Inventory Import · Fill Rate · Revenue Lost · Most Affected Accounts · Items vs Revenue · Operations Dashboard · Email Notifications · Implementation · Guardrails · Troubleshooting*
