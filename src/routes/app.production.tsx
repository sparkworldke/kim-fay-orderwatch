    ...
    <MsiProvider>
      <ProductionStockViewContext.Provider value={{ value: stockView, setValue: setStockView }}>
        <ProductionResetContext.Provider value={resetToken}>
          <section className="production-dashboard -m-3 flex min-h-[calc(100dvh-3.5rem)] flex-col bg-background sm:-m-4 md:-m-6 xl:h-[calc(100dvh-3.5rem)] xl:min-h-0 xl:overflow-hidden">
            <DashboardHeader onReset={() => setResetToken((token) => token + 1)} />
            <div className="mx-auto flex min-h-0 w-full max-w-[1800px] flex-1 flex-col overflow-y-auto px-3 py-2 sm:px-4 xl:overflow-hidden">
              <Outlet />
            </div>
          </section>
