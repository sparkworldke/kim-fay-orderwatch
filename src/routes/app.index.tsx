...
interface CustomerBrandsResponse {
  data: CustomerBrandRow[];
  meta: { current_page: number; last_page: number; per_page: number; total: number };
  summary?: PortfolioSummary;
  brand_rollup?: BrandRollupRow[];
  date_from: string;
  date_to: string;
}
...
interface DashboardFilterOptions {
  ...
  consultants: Array<{ id: number; name: string; rep_code: string }>;
  consultant_locked: boolean;
  current_consultant: { id: number; name: string; rep_code: string } | null;
}
...
function dashboardParams(dateFrom: string, dateTo: string, filters: DashboardFilters) {
  const params = new URLSearchParams({ date_from: dateFrom, date_to: dateTo });
  if (filters.customers.length) params.set("customer", filters.customers.join(","));
  if (filters.statuses.length) params.set("status", filters.statuses.join(","));
  if (filters.segments.length) params.set("segment", filters.segments.join(","));
  if (filters.brands.length) params.set("brand", filters.brands.join(","));
  if (filters.repCodes.length) params.set("rep_code", filters.repCodes.join(","));
  return params;
}
...
function useKpis(dateFrom: string, dateTo: string, filters: DashboardFilters) {
  const params = dashboardParams(dateFrom, dateTo, filters);
  return useQuery({
    queryKey: ["dashboard-kpis", dateFrom, dateTo, filters],
    queryFn: () => apiFetch<KpiData>(`dashboard/kpis?${params.toString()}`),
  });
}

function useStatusOrders(statusKey: string, day: string, enabled: boolean, filters: DashboardFilters) {
  // Day expand panels always load a single status bucket; clear multi status filters.
  const params = dashboardParams(day, day, { ...filters, statuses: [] });
  params.set("status", statusKey);
  return useQuery({
    queryKey: ["dashboard-orders-by-status", statusKey, day, filters],
    queryFn: () =>
      apiFetch<DashboardStatusOrdersResponse>(
        `dashboard/orders-by-status?${params.toString()}`,
      ),
    enabled,
  });
}
...
function useTrend(dateFrom: string, dateTo: string, compare: boolean, filters: DashboardFilters) {
  ...
  return useQuery({
    queryKey: ["dashboard-trend", dateFrom, dateTo, compare, filters],
    queryFn: () => apiFetch<TrendData>(`dashboard/trend?${params.toString()}`),
  });
}
...
function useCustomerBrands(
  ...
  if (partnerBrand) params.set("partner_brand", partnerBrand);
  if (brand) params.set("brand", brand);
  return useQuery({
    queryKey: [
      "dashboard-customer-brands",
      dateFrom,
      dateTo,
      search,
      page,
    ...
    enabled,
    // Keep previous result visible while filters refetch — no skeleton flash.
      ...
      <table className="w-full text-[8px]">
        <thead>
          <tr className="border-b bg-muted/40 text-left">
            ...
            <th className="px-1.5 py-0.5 text-right font-semibold text-muted-foreground">
              Quantity
            </th>
          </tr>
        </thead>
        ...
        {visible.map((statusKey) => {
          ...
          return (
            <AccordionItem
              key={statusKey}
              value={statusKey}
              className="border-b px-1.5 last:border-b-0"
            >
              ...
              <AccordionContent className="pb-1">
                <StatusOrderList statusKey={statusKey} day={day} enabled={isOpen} filters={filters} />
              </AccordionContent>
            </AccordionItem>
          );
        })}
...
function DailyOrderTable({ trendData, filters }: { trendData: TrendDay[]; filters: DashboardFilters }) {
    ...
    <div className="rounded border bg-card shadow-[var(--shadow-panel)]">
      ...
      {!collapsed && (
        <div className="overflow-x-auto">
          <table className="w-full text-[8px]">
            <thead>
              <tr className="border-t bg-muted/40">
                ...
                <th className="px-1.5 py-1 text-right font-semibold text-muted-foreground">
                  Total
                </th>
                {STATUS_COLS.map((col) => (
                  <th
                    key={col.key}
                    className="px-1.5 py-1 text-right font-semibold text-muted-foreground whitespace-nowrap"
                  >
                    {col.label}
                  </th>
                ))}
            ...
            <tbody>
                ...
                return (
                  <Fragment key={row.day}>
                    ...
                    >
                      ...
                      <td className="px-1.5 py-0.5 text-right tabular-nums font-semibold">
                    ...
                    {isExpanded && (
                      <tr className="border-t bg-muted/15">
                        <td colSpan={colSpan} className="px-2 py-1">
                          <DayRowOrdersPanel day={row.day} row={row} filters={filters} />
                        </td>
                      </tr>
                    )}
                  </Fragment>
              ...
              {rows.length > 0 &&
                (() => {
                  ...
                  const fillTotalsRow = {
                    fill_shipped_qty: rows.reduce(
                      (sum, row) => sum + (row.fill_shipped_qty ?? 0),
                      0,
                    ),
                    fill_ordered_qty: rows.reduce(
                      (sum, row) => sum + (row.fill_ordered_qty ?? 0),
                      0,
                    ),
          ...
          {rows.length === 0 && (
            <p className="py-4 text-center text-[8px] text-muted-foreground">
...
function StatusBadge({ statusKey, value }: { statusKey: StatusCol; value: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded border px-1 py-px text-[8px] font-medium tabular-nums ${styles[statusKey]}`}
    >
    ...
    <div className="rounded border bg-card shadow-[var(--shadow-panel)]">
      ...
      >
        <div>
          ...
          <p className="text-[8px] text-muted-foreground">
            Monthly totals with fill rate and completion rate
          </p>
        </div>
        {collapsed ? (
          <ChevronRight className="h-3 w-3 text-muted-foreground" />
                ...
                >
                  ...
                  <td className="px-1.5 py-0.5 text-right tabular-nums">
                    <FillRateBadge
                      ...
                      row={{
                        fill_shipped_qty: row.fill_shipped_qty,
                        fill_ordered_qty: row.fill_ordered_qty,
                      }}
                    />
                  </td>
                  <td className="px-1.5 py-0.5 text-right tabular-nums">
  ...
  return (
    <div className="rounded border bg-muted/30 px-2 py-1.5 text-[8px]">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
        ...
        <Badge
          variant="outline"
          className="h-4 px-1 py-0 text-[8px] font-mono tabular-nums text-amber-800 border-amber-300 dark:text-amber-300"
        >
...
function GoodsLostInTransitPanel({
  ...
  if (glt.isError) {
    return (
      <p className="text-[8px] text-destructive">Could not load Goods Lost in Transit orders.</p>
    );
  }
        ...
        <div className="grid grid-cols-3 gap-1 sm:grid-cols-4 lg:grid-cols-7">
          {statusCards.map((card) => (
            <div key={card.label} className="rounded border bg-muted/20 px-1.5 py-1">
              ...
              <p className={`text-[11px] font-bold tabular-nums leading-tight ${card.tone}`}>
...
function DashboardPage() {
  ...
  const [tab, setTab] = useState<"sales-orders" | "goods-lost" | "zone-routes" | "brands">(
    "sales-orders",
  );
  ...
  const lockedRepCode = filterOptions.data?.current_consultant?.rep_code?.trim() ?? "";
  const effectiveFilters = filterOptions.data?.consultant_locked
    ? { ...filters, repCodes: lockedRepCode ? [lockedRepCode] : [] }
    : filters;
  const kpis = useKpis(dateFrom, dateTo, effectiveFilters);
  const trend = useTrend(dateFrom, dateTo, compare, effectiveFilters);
  ...
  function toggleStat(key: StatKey) {
    setActiveStats((prev) => (prev.includes(key) ? prev.filter((k) => k !== key) : [...prev, key]));
  }
  ...
  return (
    <div className="space-y-2 text-[8px]">
      ...
      <div className="flex flex-wrap items-end justify-between gap-1.5">
        <div className="min-w-0">
          ...
          <p className="text-[8px] leading-tight text-muted-foreground">
            Sales orders (SO) only. Goods Lost in Transit (
            {kpi?.goods_lost_in_transit?.customer_id ?? "CUST102641"}) is on its own tab and
            excluded from SO totals.
          </p>
        </div>
        <div className="flex flex-wrap items-end gap-1">
          <div className="grid gap-0.5">
            ...
            <Input
              ...
              className="h-6 w-[7.5rem] px-1 text-[8px]"
            />
          </div>
          <div className="grid gap-0.5">
          ...
          >
            <RefreshCw
              className={`mr-0.5 h-3 w-3 ${orderStatusRefresh.isPending ? "animate-spin" : ""}`}
            />
          ...
          <div className="flex h-6 items-center gap-1 rounded border px-1.5">
            ...
            <Label
              ...
              className="cursor-pointer text-[8px] select-none whitespace-nowrap"
            >
              Compare prev.
            </Label>
          </div>
      ...
      <div className="flex flex-wrap items-end gap-1.5 border-y bg-muted/15 px-1.5 py-1.5">
        <MultiSelect
          label="Customer"
          compact
          className="min-w-52 max-w-80 flex-1"
          emptyLabel="All customers"
          allLabel="All customers"
          showValueHint={false}
          options={(filterOptions.data?.customers ?? []).map((customer) => {
            const name = customer.name?.trim() ?? "";
            // Never show raw CUST… id as the chip/list label when a name is present.
            const looksLikeId =
              /^CUST\d+/i.test(name) || name.toUpperCase() === customer.id.toUpperCase();
            return {
              value: customer.id,
              label: name && !looksLikeId ? name : name || customer.id,
            };
          })}
          selected={filters.customers}
          onChange={(customers) => setFilters((current) => ({ ...current, customers }))}
        ...
        <MultiSelect
          ...
          compact
          className="min-w-40"
          emptyLabel="All segments"
          allLabel="All segments"
          options={(filterOptions.data?.segments ?? []).map((segment) => ({
            value: segment.id,
            label: segment.name,
          }))}
          selected={filters.segments}
          onChange={(segments) => setFilters((current) => ({ ...current, segments }))}
        ...
        <MultiSelect
          ...
          options={(filterOptions.data?.consultants ?? []).map((consultant) => ({
            value: consultant.rep_code,
            label: consultant.name,
          }))}
          selected={
            filterOptions.data?.consultant_locked
              ? (lockedRepCode ? [lockedRepCode] : [])
              : filters.repCodes
          }
          onChange={(repCodes) => setFilters((current) => ({ ...current, repCodes }))}
          searchPlaceholder="Search consultant or rep code..."
          disabled={Boolean(filterOptions.data?.consultant_locked)}
        />
        <Button
          variant="ghost"
          size="sm"
      ...
      <Tabs
        value={tab}
        onValueChange={(v) => setTab(v as "sales-orders" | "goods-lost" | "zone-routes" | "brands")}
        className="space-y-2"
      >
        <TabsList className="h-7 gap-0.5 p-0.5">
          ...
          <TabsTrigger value="brands" className="h-6 gap-1 px-2 text-[8px]">
        ...
        <TabsContent value="sales-orders" className="mt-0 space-y-2">
            ...
            <div className="grid grid-cols-4 gap-1 sm:grid-cols-4 lg:grid-cols-8">
              {STATS.map((s) => (
                ...
                >
                  <div className="mb-0.5 flex items-center justify-between gap-0.5">
                    <span
                      className={`truncate text-[8px] font-semibold uppercase leading-tight tracking-wide ${s.color} opacity-80`}
                    >
                      {s.label}
                    </span>
              ...
              <div className="rounded border p-1.5 bg-teal-50 border-teal-200 dark:bg-teal-950/40 dark:border-teal-800">
                ...
                <div className="text-[13px] font-bold leading-tight tabular-nums text-teal-700 dark:text-teal-300">
                  {kpi
                    ? kpi.avg_per_day.toLocaleString("en-KE", {
                ...
                <div className="mt-0.5 text-[7px] leading-none text-teal-600/70 dark:text-teal-400/70">
                  {kpi
                    ? `${kpi.active_days} active day${kpi.active_days !== 1 ? "s" : ""} · MTD`
                    : "orders/day"}
                </div>
              </div>
            </div>
            ...
            <div className="mb-1.5 flex flex-wrap items-center justify-between gap-1.5">
              <div>
                ...
                <p className="text-[8px] text-muted-foreground">
                  ...
                  {compare && (
                    <span className="ml-1 text-muted-foreground/70">(dashed = prev.)</span>
                  )}
                </p>
              </div>
              {/* Series toggles */}
          ...
          {trend.isLoading ? (
            ...
            <DailyOrderTable trendData={trend.data.current} filters={effectiveFilters} />
          ) : null}
        </TabsContent>
        ...
        <TabsContent value="zone-routes" className="mt-0 space-y-2">
          <ZoneRoutesPanel dateFrom={dateFrom} dateTo={dateTo} enabled={tab === "zone-routes"} />
        </TabsContent>

        <TabsContent value="brands" className="mt-0 space-y-2">
          <CustomerBrandsPanel dateFrom={dateFrom} dateTo={dateTo} enabled={tab === "brands"} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function CustomerBrandsPanel({
  ...
  const [view, setView] = useState<"by-brand" | "by-customer">("by-brand");

  // Debounce search so typing doesn't re-hit the API every keystroke.
  useEffect(() => {
  ...
  // partner_brand already encodes manufactured vs trading; keep business_category in sync.
  const category =
    brandFilter.partner_brand === "trading"
      ? "trading"
      : brandFilter.partner_brand === "manufactured"
        ? "manufactured"
        : "all";

  const report = useCustomerBrands(
    dateFrom,
    dateTo,
    search,
    page,
  ...
  const brandRollup = report.data?.brand_rollup ?? [];
  const hasData = !!summary && (summary.customer_count > 0 || brandRollup.length > 0);
  const showEmpty = !report.isLoading && !report.isFetching && !report.isError && !hasData;

  return (
    <div className="space-y-3 rounded border bg-card p-3 text-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold">Partner Brand performance</h2>
          <p className="text-xs text-muted-foreground">
            Track partner (trading) brands by demand, delivery, fill rate, and at-risk value.
            Filter by brand group or a single brand. Switch view for brand roll-up or customer detail.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1 rounded-md border p-0.5">
            <Button
              size="sm"
              variant={view === "by-brand" ? "default" : "ghost"}
              className="h-7 px-2 text-xs"
              onClick={() => setView("by-brand")}
            >
              By brand
            </Button>
            <Button
              size="sm"
              variant={view === "by-customer" ? "default" : "ghost"}
              className="h-7 px-2 text-xs"
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <option value="all">All segments (GT / MT / KP / CS)</option>
        ...
        <Input
          ...
          placeholder="Search customer, ID, or brand…"
          className="h-8 w-full sm:w-64"
        />
        {(brandFilter.partner_brand || brandFilter.brand || searchInput || segment !== "all") && (
          <Button
            size="sm"
            variant="outline"
            className="h-8"
            onClick={() => {
              setBrandFilter({ partner_brand: "trading", brand: "", category: "" });
              setSearchInput("");
              setSearch("");
              setSegment("all");
              setPage(1);
      ...
      {report.isError ? (
        <div className="rounded border border-destructive p-4 text-destructive">
          Unable to load partner brand performance.
        </div>
      ) : hasData ? (
        <>
          ...
          {view === "by-brand" ? (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold">Brand roll-up</h3>
              <p className="text-xs text-muted-foreground">
                One row per brand across all customers in the selected filters and date range.
                Click a brand with open qty to see the undelivered SKU split (product, inventory ID, unit price, why at risk, SOs).
              </p>
              {brandRollup.length > 0 ? (
                <div className="overflow-x-auto rounded border">
                  <BrandTable rows={brandRollup} showCustomers />
                </div>
              ) : (
                <div className="rounded border p-6 text-center text-muted-foreground">
                  No brand activity for this filter.
                </div>
              )}
            </div>
          ) : (
        ...
        <div className="rounded border p-8 text-center text-muted-foreground">
          No partner brand activity for this period. Try expanding the date range or clearing the brand filter.
        </div>
      ) : (
        // First load only — compact status, no skeleton preloaders.
