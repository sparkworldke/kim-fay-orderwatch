...
function CustomerProfile({ customer }: { customer: AcumaticaCustomer }) {
  ...
  const fields: Array<[string, unknown]> = [
    ["Customer ID", customer.acumatica_id],
    ["Status", customer.status],
    ["Customer class", customer.customer_class],
    ["Customer group", extra?.customer_group],
    ["Category", extra?.category],
    ["Region", extra?.customer_region],
    [
      "Parent",
      customer.parent
        ? `${customer.parent.name} (${customer.parent.acumatica_id})`
    ...
    [
      "Credit limit",
      extra?.credit_limit
        ? `${extra.currency_id ?? "KES"} ${Number(extra.credit_limit).toLocaleString()}`
        : null,
    ],
    ["Price class", [extra?.price_class_id, extra?.price_class_name].filter(Boolean).join(" — ")],
    ["Statement", [extra?.statement_type, extra?.statement_cycle].filter(Boolean).join(" / ")],
    ["Shipping rule", extra?.shipping_rule],
    ["Delivery", extra?.delivery],
    ["Sage code", extra?.sage_code],
    ["Business account ID", extra?.business_account_id],
    ["Tax registration ID", extra?.tax_registration_id],
    ["Main A/C owner", extra?.main_ac_owner],
    ["Rep / Sales rep", [extra?.rep_code, extra?.sales_rep].filter(Boolean).join(" — ")],
    [
      "Sight consultant",
      customer.servicing_consultant
        ? `${customer.servicing_consultant.name}${customer.servicing_consultant.rep_code ? ` (${customer.servicing_consultant.rep_code})` : ""}`
        : null,
    ],
    ...
    [
      "Address",
      [
        extra?.address_line_1,
        extra?.address_line_2,
        extra?.address_line_3,
        extra?.city,
        extra?.country,
      ]
        .filter(Boolean)
    ...
    ["Company email", extra?.email ?? customer.email],
    ["Company phone", customer.phone],
    ["Source", extra?.source],
    [
      "Last synced",
      extra?.synced_at
        ? new Date(extra.synced_at).toLocaleString()
        : customer.synced_at
          ? new Date(customer.synced_at).toLocaleString()
          : null,
    ],
  ];
  return (
...
function CustomerDocumentsIndex({ customerId }: { customerId: string }) {
  ...
  const whitespace = useWhitespace(customerId);
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          {customer.data?.customer_data?.customer_group && (
            <Badge>{customer.data.customer_data.customer_group}</Badge>
          )}
          {customer.data?.customer_class && (
            <Badge variant="secondary">{customer.data.customer_class}</Badge>
          )}
        </div>
      </div>
      ...
      <CustomerContactsCard
        ...
        health={
          customer.data?.contact_health
            ? {
                hasPhone: customer.data.contact_health.has_phone,
                hasEmail: customer.data.contact_health.has_email,
                hasPrimary: customer.data.contact_health.has_primary,
              }
            : undefined
        }
      />
              ...
              <AccordionContent>
                {orders.isLoading ? (
                  ...
                  <EmptyBlock
                    message={
                      branches.length > 0
                        ? "No sales documents found directly against this account — check its branches above."
                        : "No sales documents found for this customer."
                    }
                  />
                ) : (
                  <>
